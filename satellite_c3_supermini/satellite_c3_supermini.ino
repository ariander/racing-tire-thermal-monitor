...
void compress_frame() {
    ...
    for (int y = 0; y < 6; y++) {
        for (int x = 0; x < 32; x++) {
            ...
            // Encode temperatur til 0-255 byte
            // Range: 0-127°C (0.5°C per step)
            // Dette gir oss: 0°C=0, 25°C=50, 70°C=140, 100°C=200, 127°C=254
            int val = (int)(avg * 2.0);  // 0.5°C resolution
            if (val < 0) val = 0;
            if (val > 255) val = 255;
